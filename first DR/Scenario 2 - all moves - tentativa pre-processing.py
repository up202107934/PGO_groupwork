...
Author: Joana (preprocessing upgraded)
"""

# ============================================================
# 0) Imports + seeds
#    Purpose: ensure reproducibility and load all dependencies.
# ============================================================
...
# ============================================================
# 1) Parameters
#    Purpose: configure the algorithm and constants.
# ============================================================
DATA_FILE = "Instance_MKP2.dat"

C_PER_SHIFT = 360   # minutes per shift (6h * 60)
CLEANUP = 17        # cleaning time (minutes)
...
ALPHA5 = 1/3   # fit term
ALPHA6 = 1/3   # early day term
ALPHA7 = 1/3   # continuity term

TOLERANCE = 15        # allowed overtime per block (minutes)
ROOM_CHANGE_TIME = 5  # surgeon room change time (minutes)

# ============================================================
# 2) Robust instance reader (preprocessing + flexible naming)
#    Purpose: read different .dat formats and normalize to a common schema.
# ============================================================

# --- Name aliases for integers ---
INT_ALIASES = {
    "n_patients": ["NumberPatients", "NumPatients", "Patients", "NPatients", "nPatients"],
    "n_rooms":    ["NumberOfRooms", "NumRooms", "Rooms", "NRooms", "nRooms"],
    "n_days":     ["NumberOfDays", "NumDays", "Days", "NDays", "nDays"],
    "n_surgeons": ["NumberSurgeons", "NumberOfSurgeons", "NumSurgeons", "Surgeons", "NSurgeons", "nSurgeons"],
}

# --- Name aliases for arrays ---
ARR_ALIASES = {
    "Duration":            ["Duration", "Durations", "ProcTime", "ProcessingTime", "SurgeryDuration"],
    "Priority":            ["Priority", "Priorities", "Urgency", "Triage", "ClinicalPriority"],
    "Waiting":             ["Waiting", "Wait", "WaitingDays", "DaysWaited", "ElapsedDays"],
    "Surgeon":             ["Surgeon", "SurgeonsOfPatients", "AssignedSurgeon", "SurgeonId", "Doctor", "Physician"],
    "BlockAvailability":   ["BlockAvailability", "RoomAvailability", "ORAvailability", "BlocksOpen", "RoomOpen"],
    "SurgeonAvailability": ["SurgeonAvailability", "DoctorAvailability", "PhysicianAvailability", "SurgeonsAvailability"],
}
...
def _fix_ellipsis(s: str) -> str:
    """
    Handle cases like [1,2,...,5] by removing the ellipsis token.
    Note: if the file is truly truncated (missing closing brackets), parsing will still fail.
    """
    s = re.sub(r",\s*\.\.\.\s*,", ",", s)
    s = re.sub(r"\[\s*\.\.\.\s*,", "[", s)
    s = re.sub(r",\s*\.\.\.\s*\]", "]", s)
...
def _fix_trailing_commas(s: str) -> str:
    """Remove trailing commas before a closing bracket: [1,2,] -> [1,2]."""
    s = re.sub(r",\s*\]", "]", s)
    return s

def _extract_rhs(text: str, varname: str) -> str | None:
    """Extract the RHS of 'varname = ... ;' including multiline arrays."""
    m = re.search(rf"\b{re.escape(varname)}\s*=\s*([\s\S]*?)\s*;", text)
    return m.group(1).strip() if m else None

def _try_get_int(text: str, candidates: list[str]) -> int | None:
    """Try multiple names to find an int declaration."""
    for name in candidates:
        m = re.search(rf"\bint\s+{re.escape(name)}\s*=\s*(\d+)", text)
        if m:
            return int(m.group(1))
    # Some formats omit 'int' keyword: name = 123;
    for name in candidates:
        rhs = _extract_rhs(text, name)
        if rhs and re.fullmatch(r"\d+", rhs.strip()):
            return int(rhs.strip())
    return None

def _parse_python_literal(rhs: str):
    """Parse list/nested-list literals safely after small cleanups."""
    rhs = _fix_trailing_commas(_fix_ellipsis(rhs))
    return ast.literal_eval(rhs)

def _parse_flat_int_list(rhs: str) -> list[int]:
    """Fallback parse: extract integers from any RHS string."""
    nums = re.findall(r"-?\d+", rhs)
    return [int(x) for x in nums]

def _try_get_array(text: str, candidates: list[str]):
    """Try multiple names to find an array; parse with literal_eval or fallback."""
    for name in candidates:
        rhs = _extract_rhs(text, name)
        if rhs is None:
            continue
        try:
            return _parse_python_literal(rhs)
        except Exception:
            # This fallback is only reliable for flat lists (Duration/Priority/Waiting/Surgeon)
            return _parse_flat_int_list(rhs)
    return None

def read_instance_any(path: str) -> dict:
    """
    Read an instance file with flexible variable names and normalize fields:
      - n_patients, n_rooms, n_days, n_surgeons
      - Duration, Priority, Waiting, Surgeon
      - BlockAvailability, SurgeonAvailability
    """
    raw = Path(path).read_text(encoding="utf-8", errors="replace")

    # --- Preprocess raw text (remove comments and common formatting issues) ---
    text = _strip_comments(raw)
    text = _fix_ellipsis(text)
    text = _fix_trailing_commas(text)

    # --- Read integers with alias support ---
    n_patients = _try_get_int(text, INT_ALIASES["n_patients"])
    n_rooms    = _try_get_int(text, INT_ALIASES["n_rooms"])
    n_days     = _try_get_int(text, INT_ALIASES["n_days"])
    n_surgeons = _try_get_int(text, INT_ALIASES["n_surgeons"])

    # --- Read arrays with alias support ---
    Duration            = _try_get_array(text, ARR_ALIASES["Duration"])
    Priority            = _try_get_array(text, ARR_ALIASES["Priority"])
    Waiting             = _try_get_array(text, ARR_ALIASES["Waiting"])
    Surgeon             = _try_get_array(text, ARR_ALIASES["Surgeon"])
    BlockAvailability   = _try_get_array(text, ARR_ALIASES["BlockAvailability"])
    SurgeonAvailability = _try_get_array(text, ARR_ALIASES["SurgeonAvailability"])

    # ============================================================
    # 3) Normalize / infer missing fields
    #    Purpose: ensure your algorithm always receives the same schema.
    # ============================================================

    # --- Duration is mandatory for the scheduling problem ---
    if Duration is None:
        raise ValueError(f"{path}: Missing Duration (or aliases). Cannot run scheduling.")

    # --- Infer n_patients from Duration if missing ---
    if n_patients is None:
        n_patients = len(Duration)

    # --- Fill missing clinical attributes with safe defaults ---
    if Priority is None:
        Priority = [0] * n_patients
        warnings.warn(f"{path}: Missing Priority -> defaulting to 0 for all patients.")

    if Waiting is None:
        Waiting = [0] * n_patients
        warnings.warn(f"{path}: Missing Waiting -> defaulting to 0 for all patients.")

    # --- Surgeon per patient: default to 1 if missing ---
    if Surgeon is None:
        if n_surgeons is None:
            n_surgeons = 1
        Surgeon = [1] * n_patients
        warnings.warn(f"{path}: Missing Surgeon mapping -> defaulting surgeon_id=1 for all patients.")

    # --- Infer n_surgeons from Surgeon list if missing ---
    if n_surgeons is None:
        n_surgeons = max(Surgeon) if len(Surgeon) else 1

    # --- Infer n_days / n_rooms from BlockAvailability if missing ---
    if (n_days is None or n_rooms is None) and isinstance(BlockAvailability, list):
        try:
            n_days = n_days or len(BlockAvailability)
            n_rooms = n_rooms or len(BlockAvailability[0])
        except Exception:
            pass

    # --- If still missing, we cannot build blocks ---
    if n_days is None or n_rooms is None:
        raise ValueError(f"{path}: Could not infer NumberOfDays/NumberOfRooms (and no availability matrix).")

    # --- Default to all-open availability if missing ---
    if BlockAvailability is None:
        BlockAvailability = [[[1, 1] for _ in range(n_rooms)] for __ in range(n_days)]
        warnings.warn(f"{path}: Missing BlockAvailability -> defaulting all rooms open in all shifts.")

    if SurgeonAvailability is None:
        SurgeonAvailability = [[[1, 1] for _ in range(n_days)] for __ in range(n_surgeons)]
        warnings.warn(f"{path}: Missing SurgeonAvailability -> defaulting all surgeons available in all shifts.")

    # --- Essential consistency checks ---
    if not (len(Duration) == len(Priority) == len(Waiting) == len(Surgeon) == n_patients):
        raise ValueError(
            f"{path}: Inconsistent patient array lengths. "
            f"n_patients={n_patients}, "
            f"len(Duration)={len(Duration)}, len(Priority)={len(Priority)}, "
            f"len(Waiting)={len(Waiting)}, len(Surgeon)={len(Surgeon)}"
        )
    ...
    return {
        ...
        "raw_cleaned_text": text,  # handy for debugging
    }

# ============================================================
# 4) Load data (normalized schema)
#    Purpose: create the exact variables your current code expects.
# ============================================================
data = read_instance_any(DATA_FILE)

n_patients = data["n_patients"]
durations  = data["Duration"]
priorities = data["Priority"]
waitings   = data["Waiting"]
surgeons   = data["Surgeon"]

n_rooms = data["n_rooms"]
n_days  = data["n_days"]
block_av = data["BlockAvailability"]

n_surgeons = data["n_surgeons"]
surg_av = data["SurgeonAvailability"]
...
# =========================================================
#       FIRST LOCAL SEARCH - swap i - j (REFATORADO)
# =========================================================
...
for it in range(N_ILS_ITER):
    ...
    # remove stale clinical columns if they exist
    drop_cols = [c for c in ["duration", "priority", "waiting"] if c in neighbor_struct.columns]
    
    neighbor_base = neighbor_struct.drop(columns=drop_cols)
    
    neighbor_enriched = neighbor_base.merge(
        df_patients[["patient_id", "duration", "priority", "waiting"]],
        on="patient_id",
        how="left"
    )
    ...
    neighbor_enriched = sequence_global_by_surgeon(
        neighbor_enriched,
        C_PER_SHIFT=C_PER_SHIFT,
        CLEANUP=CLEANUP,
        TOLERANCE=TOLERANCE,
    ...
    neighbor_seq = neighbor_enriched[
        neighbor_enriched["scheduled_by_seq"] == 1
    ].sort_values(
        ["day", "shift", "room", "seq_in_block"]
    ).reset_index(drop=True)
    
    # evaluation ONLY on sequenced solution
    ...
    if neigh_score > current_score:
        ...
        log_iteration(
            "ILS1_SWAP",
            it,
            eval_components(neighbor_seq, neigh_rooms_free, neigh_feas),
...
for it in range(N_ILS2_ITER):
    ...
    drop_cols = [c for c in ["duration", "priority", "waiting"] if c in neighbor.columns]
    neighbor_base = neighbor.drop(columns=drop_cols)
...
for it in range(N_ILS_ITER):
    ...
    neigh_enriched2 = neighbor_assignments_ls4.merge(
        df_patients[["patient_id","duration","priority","waiting"]],
        on="patient_id", how="left"
    )

    neigh_enriched2 = sequence_global_by_surgeon(
        neigh_enriched2,
        C_PER_SHIFT=C_PER_SHIFT,
        CLEANUP=CLEANUP,
        TOLERANCE=TOLERANCE,
    ...
    neighbor_seq = neigh_enriched2[neigh_enriched2["scheduled_by_seq"]==1].sort_values(
        ["day","shift","room","seq_in_block"]
    ).reset_index(drop=True)
